...
# ROC curve
val_preds = predict(model, data = as.matrix(valid[, colnames(valid) != "suicide_attack"]), n = model$best_iter)

#plot ROC
roc(valid$suicide_attack, val_preds, plot = TRUE, col = "steelblue", lwd = 3, 
    levels=base::levels(as.factor(valid$suicide_attack)), grid=TRUE)
