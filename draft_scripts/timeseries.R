...
fit_arima <- auto.arima(dfts)
fit_nn <- nnetar(dfts, repeats = 10)
fit_tbats <- tbats(dfts)
fit_ets <- ets(dfts)
...
tbl_arima           <- timetk::tk_tbl(fc_arima$mean) 
tbl_nn              <- timetk::tk_tbl(fc_nn$mean)
tbl_tbats           <- timetk::tk_tbl(fc_tbats$mean)
tbl_ets             <- timetk::tk_tbl(fc_ets$mean)

tbl <- tbl_arima %>% left_join(tbl_nn, by = "index") %>% left_join(tbl_tbats, by = "index") %>% left_join(tbl_ets, by = "index")
names(tbl)    <- c("time_period", "Auto_Arima", "NeuralNet", "TBATS", "ETS")
tbl$ensemble <- rowMeans(tbl[,2:5])


ens <- ts(tbl[, 6], start = Year(min(tbl$time_period)), frequency = 12)

plot_ly() %>%
  add_lines(x = time(dfts), y = dfts,
            color = I("#487caf"), name = "actual") %>%
  add_lines(x = time(ens), y = ens, color = I("orange"), name = "ensemble") %>% 
  layout(legend = list(x = 0.1, y = 0.9),
         shapes = list(list(type = "rect",
                      fillcolor = "orange", line = list(color = "orange"), opacity = 0.2,
                      x0 = Year(min(tbl$time_period)), x1 = Year(max(tbl$time_period)) + 1, xref = "x",
                      y0 = 0, y1 = max(dfts), yref = "y")))
