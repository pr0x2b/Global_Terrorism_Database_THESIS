...
From the top five patterns based on confidence, we can see that use of chemical weapon turns out to be the most frequent pattern with relatively high lift value. It is also interesting to see that attacks on other terrorists (non state militia) is observed in 3 out of top 5 patterns.  
...
From the plot above, we can identify many interesting patterns with confidence above 0.55 with high support such as attacks on NGO and government officials however most patterns indicates attack on police only. Let us have a detailed look at all the patterns with network graph. 
...
In total, 27 rules are identified after removing redundant rules. Let's have a closer look at all the 27 rules with network graph to visualize the characteristics and habits of Boko Haram group.
