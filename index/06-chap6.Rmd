...
Based on exploratory data analysis, it is observed that the number attacks with visible pattern began from year 2000 so the data is selected between year 2000 to 2016. To get the time-series frequency by months for all the years, I have added missing months and assigned zero as shown in the code below: 
...
```
Decomposition by additive and multiplicative time-series is helpful to understand anomalies in data. Based on observed patterns, this decomposition comprises three components [@Hyndman_2018]: 
...
```
Although we can spot outliers in all nine lags however linear pattern is visible. To further explain this, we can see positive linear trend (going upward from left to right) which is an indication that positive auto-correlation is present. Specifically, lags 1, 2, 3 and 9 shows strong positive auto-correlation. Presence of auto-correlation can be problematic for some models. 


### Modelling

In this part of the analysis, I split the data in training and test set in order to evaluate performance of four different models before making the actual forecasts. 
...
```{r echo= FALSE, fig.height=4, out.width="100%", fig.cap="Auto Arima: Residuals"}
# plot the residuals
if( knitr:::is_latex_output() ) {
...
```
From the plot above, it is observed that Auto Arima model nearly captures fitted values based on training data but forecasted values are little bit apart from actual values (test data- year 2016). Next, we examine the pattern in actual vs fitted and forecasted values for remaining three models. 
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="Auto Arima Forecast (Afghanistan)"}
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="Neural Network Forecast (Afghanistan)"}
# combine plots for latex output
if( knitr:::is_latex_output() ) {
  plot(fore_nn)
}
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="TBATS Forecast (Afghanistan)"}
# combine plots for latex output
if( knitr:::is_latex_output() ) {
  plot(fore_tbats)
}
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="ETS Forecast (Afghanistan)"}
# combine plots for latex output
if( knitr:::is_latex_output() ) {
  plot(fore_ets)
}
...
The forecasting results are often represented by mean value and by confidence interval of 80% and 95%. The mean value of forecast is considered as final forecasting value. Next, we extract forecasts for chosen horizon and add the ensembled predictions to be used as insights as predicted future attacks in Afghanistan as shown in the code chunk below:
...
Seasonal components within year indicates high number of fatalities in July followed by April and May. An interesting observation from the second plot above is that the variation in number fatalities by months before 2008 and after 2013 is significantly different. 
...
In statistical terms, correlation means the extent of a linear relationship between two variables. Same way, auto-correlation means the linear relationship between lagged values of a time series as shown in the plot above for nine lags. We can spot few outliers in all nine lags as well as positive linear trend indicates the presence of auto-correlation. 
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="Auto Arima Forecast (Iraq)"}
# combine plots for latex output
if( knitr:::is_latex_output() ) {
  plot(fore_arima)
}
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="Neural Network Forecast (Iraq)"}
# combine plots for latex output
if( knitr:::is_latex_output() ) {
  plot(fore_nn)
}
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="TBATS Forecast (Iraq)"}
# combine plots for latex output
if( knitr:::is_latex_output() ) {
  plot(fore_tbats)
}
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="ETS Forecast (Iraq)"}
# combine plots for latex output
if( knitr:::is_latex_output() ) {
  plot(fore_ets)
}
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="Auto Arima Forecast (SAHEL Region)"}
# combine plots for latex output
if( knitr:::is_latex_output() ) {
  plot(fore_arima)
}
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="Neural Network Forecast (SAHEL Region)"}
# combine plots for latex output
if( knitr:::is_latex_output() ) {
  plot(fore_nn)
}
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="TBATS Forecast (SAHEL Region)"}
# combine plots for latex output
if( knitr:::is_latex_output() ) {
  plot(fore_tbats)
}
...
```{r echo= FALSE, fig.height= 4, out.width="100%", fig.cap="ETS Forecast (SAHEL Region)"}
# combine plots for latex output
if( knitr:::is_latex_output() ) {
  plot(fore_ets)
}
