...
From the frequency table and plot, we can see that 11.3% incidents were observed as suicide attacks out of total 21,780 observations. Our objective is to train the classifier on training data (upto 2015) and correctly classify the instances of "Yes" in suicide attack variable in test data (year 2016). 
...
knitr::kable(t(tmp_test[2, ]), booktabs= TRUE,
             caption = "Actual values in 2nd observation in test set") %>%
  kable_styling(latex_options = "HOLD_position", font_size = 12, full_width = F)
```

Next, we extract the predicted value from the model and use lgb.interpret function to compute feature contribution components of raw score prediction. 
